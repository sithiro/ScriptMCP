...
Examples:
  call_process(name="watchlist_show", arguments={"name":"tech"}, terminal="window") — shows a watchlist in a new window, agent sees no output.
  call_process(name="watchlist_correlation_matrix", arguments={"showMatrix":true}, terminal="self") — shows matrix in agent's tab, agent sees no data.
  Three parallel calls with terminal="tabs" open 3 tabs in the named window simultaneously.
